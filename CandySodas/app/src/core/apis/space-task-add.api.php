...
$requiredFields = ['name', 'description', 'spaceDeadline', 'priority', 'id', 'projectId', 'status', 'projectId', 'spaceId','supplierId'];
$errors = [];
...
if (file_put_contents($jsonFilePath, json_encode($existingData, JSON_PRETTY_PRINT))) {
    echo json_encode(['success' => true, 'message' => 'Space Task saved successfully.', 'data' => $newSpace]);
} else {
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Failed to save space task.']);
}
