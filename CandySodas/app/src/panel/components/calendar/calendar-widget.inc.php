<?php 
  ...
  function gerarCalendario() {
    $diasSemana = ['Sun' => 'Do', 'Mon' => 'Mo', 'Tue' => 'Tu', 'Wed' => 'We', 'Thu' => 'Th', 'Fri' => 'Fr', 'Sat' => 'Sa'];
    $html = '<ul class="nav nav-stretch nav-pills nav-pills-custom nav-pills-active-custom d-flex justify-content-between mb-8 px-5">';

    for ($i = 0; $i < 11; $i++) {
        $data = strtotime("+$i days"); // Calcula a data
        $diaSemana = date('D', $data); // Obtém o dia da semana (ex: Mon)
        $dia = date('d', $data); // Obtém o dia do mês (ex: 09)

        $abreviacao = $diasSemana[$diaSemana]; // Converte para abreviação PT-BR

        // Adiciona a classe 'active' apenas ao primeiro dia (dia atual)
        $activeClass = ($i === 0) ? 'active' : '';

        $html .= '
            <li class="nav-item p-0 ms-0">
                <a class="nav-link btn d-flex flex-column flex-center rounded-pill min-w-45px py-4 px-3 btn-active-danger '.$activeClass.'" data-bs-toggle="tab" href="projects.html#kt_timeline_widget_3_tab_content_'.$i.'">
                    <span class="fs-7 fw-semibold">'.$abreviacao.'</span>
    ...
    <div class="card h-md-100">
      <div class="card-header border-0 pt-5">
          <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bold text-gray-900"> Calendar </span>
            <span class="text-muted mt-1 fw-semibold fs-7"> Organize our publications and tasks here </span>
          </h3>
          <div class="card-toolbar">   
            <a href="calendar.html" class="btn btn-sm btn-light"> Show Calendar </a>      
          </div>
      </div>
