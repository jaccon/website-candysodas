...
$jsonFilePath = $CONFIG['CONF']['cacheDir'].'/pages.json';
$datas = [];
...
if (file_exists($jsonFilePath)) {
    ...
    if (is_array($alldatas)) {
        foreach ($alldatas as $data) {
            if (isset($data['authorUserGroup']) && $data['authorUserGroup'] === 'admin' || $data['userId'] === 'e641456c-9fe5-11ef-b864-0242ac120002') {
                $datas[] = $data;
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    if ($itemId === Auth::getUserData($_SESSION['user'], "id")) {
        echo "<script>alert('Você não pode remover um registro que está logado'); window.location.href = 'posts.html';</script>";
        exit();
    }

    if ($itemIndex !== false && Auth::getUserData($_SESSION['user'], "usergroup") === "admin") {
      ...
      echo "<script>window.location.href = 'pages.html';</script>";
      exit();
    } else {
        echo "<script>alert('Operação não permitida. Apenas usuários do grupo \"admin\" podem ser manipulados.'); window.location.href = 'pages.html';</script>";
        exit();
    }
            ...
            <table class="table table-row-dashed align-middle gs-0 gy-3 my-0">
                <thead>
                  <tr class="fs-7 fw-bold text-gray-500 border-bottom-0">
                      ...
                      <th class="p-0 pb-3 min-w-100px text-end"> Author </th>
                          ...
                          <tr>
                              <td>
                                <div class="d-flex align-items-center">
                                    <div class="d-flex justify-content-start flex-column">
                                      <a href="page-update.html?id=<?= $item['id']; ?>" class="text-gray-800 fw-bold text-hover-primary mb-1 fs-6">
                              ...
                              <td class="text-end pe-0">
                                <span class="text-gray-600 fw-bold fs-6">
                                  <?php 
                                    $userName = Auth::getUserDataById($item['userId'], "name");
                                    echo $userName;
                                  ?>
                                </span>                                
                              </td>
