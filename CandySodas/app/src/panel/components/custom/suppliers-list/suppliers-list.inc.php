...
if (file_exists($jsonFilePath)) {
    ...
    if (is_array($alldatas)) {
        foreach ($alldatas as $data) {
            if (isset($data['usergroup']) && $data['usergroup'] === 'supplier') {
                $datas[] = $data;
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    // Verifica se o item pertence ao grupo "customer" antes de excluir
    if ($itemIndex !== false && isset($projectsData[$itemIndex]['usergroup']) && $projectsData[$itemIndex]['usergroup'] === 'customer') {
        ...
        echo "<script>alert('Operação não permitida. Apenas usuários do grupo \"customer\" podem ser manipulados.'); window.location.href = 'users.html';</script>";
        exit();
    }
}
            ...
            <table class="table table-row-dashed align-middle gs-0 gy-3 my-0">
                <thead>
                  <tr class="fs-7 fw-bold text-gray-500 border-bottom-0">
                      <th class="p-0 pb-3 min-w-175px text-start">  <?= USERS_TITLE; ?> </th>
                              ...
                              <td>
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="d-flex justify-content-start flex-column">
                                      <a href="suppliers-update.html?id=<?= $item['id']; ?>" class="text-gray-800 fw-bold text-hover-primary mb-1 fs-6">
